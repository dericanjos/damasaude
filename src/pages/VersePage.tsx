...
export default function VersePage() {
  ...
  return (
    ...
    >
      ...
      <div
        ref={cardRef}
        className="flex flex-col items-center justify-center text-center max-w-sm rounded-2xl p-8 overflow-hidden"
        style={{ border: '1px solid rgba(212,175,55,0.3)' }}
      >
        <p className={`font-medium text-foreground leading-relaxed italic text-center ${verse.verse_text.length > 200 ? 'text-xs' : verse.verse_text.length > 150 ? 'text-sm' : verse.verse_text.length > 100 ? 'text-base' : 'text-lg'}`}>
